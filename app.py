...
if parametric_file:
    try:
        ...
        df = parametric_df.copy()

        st.success(f"Processing complete. Processed rows: {len(df)}")

        with st.expander("Preview processed parametric data"):
            st.dataframe(parametric_df.head())
        with st.expander("Preview processed modeling data"):
    ...
    st.info("Upload Audi_parametric_data.xlsx to begin.")
